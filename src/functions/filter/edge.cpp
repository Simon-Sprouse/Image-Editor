...
namespace filter::edge { 
    ...
    void sobelFilter(const Image<RGB>& src, Image<RGB>& dest_grad_x, Image<RGB>& dest_grad_y) {
        ...
        dest_grad_x = Image<RGB>(w, h);
        dest_grad_y = Image<RGB>(w, h);
        ...
        for (int y = 1; y < h - 1; ++y) {
            for (int x = 1; x < w - 1; ++x) {
                ...
                dest_grad_x.setPixel(x, y, RGB(gxVal, gxVal, gxVal));
                dest_grad_y.setPixel(x, y, RGB(gyVal, gyVal, gyVal));
            }
        }
    }

    void visualizeSobel(const Image<RGB>& gradX, const Image<RGB>& gradY, Image<RGB>& dest) {
        int width = gradX.getWidth();
        int height = gradX.getHeight();
        dest = Image<RGB>(width, height);
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                dest.setPixel(x, y, RGB(val, val, val));
            }
        }
    }
    ...
    void cannyFilter(Image<RGB>& src_blurred, Image<RGB>& dest, int canny_threshold_1, int canny_threshold_2) {
    ...
    std::vector<float> computeDistanceField(const Image<RGB>& strokes_img_source) {
        ...
        //         distance_map_dest.at(x, y) = RGB(val, val, val);
        //     }
        // }
    }
